...
pub static TOOLS: &[ForensicTool] = &[
    ...
    ForensicTool {
        ...
        why_it_matters: "WHOIS is the first query an OSINT investigator runs on any domain. Even when registrant details are redacted, the registrar + registration date fingerprint the operator's tooling and timeline — short-lived domains and bulk-registered sets are strong indicators of phishing or fraud infrastructure. Free, passive, and leaves no trace on the target.",
        feeds_into: &["SpiderFoot", "Amass", "theHarvester"],
        consumes_from: &[],
        reference: Some("https://en.wikipedia.org/wiki/WHOIS"),
        environment_setup: &[],
        reproduction_steps: &[],
        verification_steps: &[],
    },
    ForensicTool {
        ...
        why_it_matters: "The attack surface of an organization is almost always bigger than its flagship domain. Subfinder surfaces the full DNS footprint — every forgotten subdomain is a potential entry point for attackers and a piece of evidence for investigators mapping infrastructure. Purely passive, so the target never sees the reconnaissance.",
        feeds_into: &["theHarvester", "SpiderFoot", "Amass"],
        consumes_from: &["whois"],
        reference: Some("https://github.com/projectdiscovery/subfinder"),
        environment_setup: &[],
        reproduction_steps: &[],
        verification_steps: &[],
    },
    ForensicTool {
        ...
        aliases: &["onionsearch"],
        category: ToolCategory::Osint,
        description: "A Python OSINT tool that runs a keyword search across 15+ dark-web search engines (Ahmia, Torch, DarkSearch, Haystak, Phobos, etc.) and returns every onion URL that mentions the keyword. Runs inside Tor so the queries themselves are anonymous, and the dark-web engines don't know who's looking.",
        typical_findings: &[
        ...
        why_it_matters: "A surface-web OSINT sweep will never see the dark web. OnionSearch is how you find out whether your target is being discussed, sold, or leaked on Tor-hidden services. Critical for fraud, breach, and threat-intelligence investigations where the stolen data often lands on onion sites before anywhere else.",
        feeds_into: &["SpiderFoot"],
        consumes_from: &[],
        reference: Some("https://github.com/megadose/OnionSearch"),
        environment_setup: &["Tor daemon running on the scanning host (onionsearch routes through the local SOCKS proxy)"],
        reproduction_steps: &[],
        verification_steps: &[],
    },
    ForensicTool {
        ...
        category: ToolCategory::Osint,
        description: "A Python dark-web OSINT scanner that searches Ahmia and a set of additional dark-web sources for mentions of a keyword, name, or domain and returns the matching onion URLs. Complementary to onionsearch — similar problem, different source set, so running both gives broader dark-web coverage.",
        typical_findings: &[
        ...
        why_it_matters: "Complements OnionSearch — the dark-web search engines each index different subsets of the onion network, so a single tool can miss sites another would find. Using both produces the broadest possible dark-web OSINT sweep for a given keyword.",
        feeds_into: &["SpiderFoot"],
        consumes_from: &[],
        reference: Some("https://github.com/josh0xA/darkdump"),
        environment_setup: &["Tor daemon running on the scanning host (darkdump routes through the local SOCKS proxy)"],
        reproduction_steps: &[],
        verification_steps: &[],
    },
];
