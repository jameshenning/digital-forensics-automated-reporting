...
pub struct PersonIdentifier {
    ...
    /// run. Carries the tool that surfaced the identifier (e.g. `"sherlock"`,
    /// `"holehe"`). `NULL` for identifiers entered manually by the
...
mod tests {
    ...
    async fn manual_add_leaves_discovered_via_tool_null() {
        ...
        let added = add_identifier(&pool, entity_id, &make_input("email", "manual@x.com"))
            .await
            .unwrap();
